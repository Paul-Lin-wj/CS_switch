...
//! 由被调脚本负责（对 8000 与真实目录失败关闭）；退 app 默认停代理、保留沙箱。
...
const REAL_SCIENCE_PORT: u16 = 8000;

/// Locate the Linux `claude-science` binary.
/// Priority: SCIENCE_BIN env → ~/.local/bin → /usr/bin → /usr/local/bin → /opt → PATH.
fn science_bin() -> Option<PathBuf> {
    if let Ok(env_bin) = std::env::var("SCIENCE_BIN") {
    ...
    let candidates: Vec<Option<PathBuf>> = vec![
        home.as_ref().map(|h| h.join(".local/bin/claude-science")),
        Some(PathBuf::from("/usr/bin/claude-science")),
        Some(PathBuf::from("/usr/local/bin/claude-science")),
        Some(PathBuf::from("/opt/claude-science/claude-science")),
    ];
...
/// 定位「资源根」（含 proxy/、scripts/）。打包成 AppImage/deb 后 bundle 进安装资源目录；
...
/// 用系统浏览器打开 URL（Linux `xdg-open`）。校验退出码：非零视为失败（P2c）。
...
fn start_proxy_for(
    ...
    let py = proc::find_exe("python3")
        .ok_or("缺少依赖 python3（起翻译代理需要）。已查 PATH、常见目录与登录 shell 仍未找到；Linux 请通过系统包管理器安装 python3（如 apt install python3）。")?;
    ...
    lifecycle.with_serialized(|| {
        ...
        if mode == "official" {
            ...
            stop_sandbox_inner(&app, &mut st).map_err(|e| {
                format!("停止沙箱失败，未切换到官方模式：{e}（真实实例 8000 未受影响）")
            })?;
            kill_child(&mut st.proxy);
            st.secret.clear();
            st.provider.clear();
...
fn open_official() -> Result<(), String> {
    let bin = science_bin().ok_or("找不到 Claude Science 二进制。请确认已安装，或设置 SCIENCE_BIN 环境变量。")?;
    ...
    match cmd.status() {
        ...
        Ok(_) => Err("未能打开 Claude Science。".into()),
...
/// 与新端口不一致；此处把这条陈旧链路拆掉（只停我们的沙箱、绝不碰 8000），逼下次「一键开始」按新端口重建，
...
fn set_settings(
    ...
    if cfg.proxy_port == REAL_SCIENCE_PORT || cfg.sandbox_port == REAL_SCIENCE_PORT {
        return Err("端口 8000 是真实 Science 实例保留端口，不能用。".into());
    }
    ...
    lifecycle.with_serialized(|| {
        ...
        if teardown {
            ...
            stop_sandbox_inner(&app, &mut st).map_err(|e| {
                format!(
                    "端口未更改：无法停止指向旧端口的沙箱（{e}），为避免留下失效链路，端口保持不变。请手动停止沙箱或重启 app 后重试。（真实实例 8000 未受影响）"
                )
            })?;
...
fn set_active_profile_txn(
    ...
    // 1) scratch 校验候选（临时端口+secret+候选 key，避开 8000；绝不碰正式链路）。
...
fn stop_all(
    ...
    lifecycle.with_serialized(|| {
        ...
        st.provider.clear();
        st.key_fp = 0;
        sandbox_res.map_err(|e| format!("代理已停；但{e}真实实例 8000 未受影响。"))
    })
}
