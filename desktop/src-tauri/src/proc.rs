...
/// 在 PATH 里找可执行文件（简易 which），并对 Linux 桌面/GUI 最小 PATH 做兜底。
///
/// 从 .desktop / AppImage 启动的 GUI 进程拿到的是最小 PATH（`/usr/bin:/bin:/usr/sbin:/sbin`），
/// **不含** `~/.local/bin`、nvm / volta / asdf / snap / flatpak 等
/// node/python 常见安装位置 → `which("node")` 在正常 PATH 里可能查不到（`python3` 因
...
/// Linux 上 node/python 等的常见安装目录（补充系统最小 PATH 未覆盖的 `~/.local/bin` 等）：
/// `/usr/local/bin`、snap、volta、asdf、`~/.local/bin`，
/// 以及 nvm 各版本 `~/.nvm/versions/node/<ver>/bin`（目录枚举）。
fn common_bin_dirs() -> Vec<PathBuf> {
    let mut dirs = vec![
        PathBuf::from("/usr/local/bin"), // 手动安装 / 发行版包
        PathBuf::from("/snap/bin"),      // snap 包
    ];
...
///
/// Linux 桌面/GUI 从 .desktop 启动时只有最小 PATH，且用户可能用 fnm / nvm / asdf 等在
/// `.bashrc` / `.bash_profile` 里配置的版本管理器（[`common_bin_dirs`] 的静态枚举覆盖不到）。
/// 这里跑 `bash -lic 'command -v <name>'`（登录 + 交互 shell，会 source 用户 rc）拿其真实
