...
# ~/.claude-science。真实实例只通过 lsof 记录 8000 的监听 PID，并在每个
# 阶段比较是否保持不变。
set -euo pipefail
...
BASELINE="$STATE_DIR/port-8000.pids"
...
# ~/.csswitch/config.json、或 STATE_DIR 里被软链的 port-8000.now）。**注意：这消除的是「跟随预置
...
validate_ports() {
  ...
  [ "$PROXY_PORT" -ne 8000 ] || die "代理端口命中真实实例保留端口 8000"
  [ "$SANDBOX_PORT" -ne 8000 ] || die "沙箱端口命中真实实例保留端口 8000"
...
assert_real_unchanged() {
  [ -f "$BASELINE" ] || die "缺少 8000 基线；先运行 preflight"
  local now="$STATE_DIR/port-8000.now"
  listener_pids 8000 | write_fresh "$now" # 删任何预置软链后写全新文件，绝不经软链写真实文件
  if ! cmp -s "$BASELINE" "$now"; then
    echo "基线 PID: $(tr '\n' ' ' <"$BASELINE")" >&2
    echo "当前 PID: $(tr '\n' ' ' <"$now")" >&2
    die "真实 Science 8000 监听发生变化，立即停止验收"
  fi
  pass "真实 Science 8000 监听 PID 保持不变"
}
...
preflight() {
  ...
  listener_pids 8000 | write_fresh "$BASELINE" # 同理：删软链后写全新基线文件
