...
# 7.7 端口归一化 + dry-run
out="$(SANDBOX_HOME="$T/vh" "$ROOT/scripts/launch-virtual-sandbox.sh" --port 08000 --dry-run 2>&1)"; rc=$?
if [ $rc -ne 0 ] && echo "$out" | grep -q "拒绝"; then ok "08000 rejected via int-normalize"; else no "08000 bypassed guard (rc=$rc)"; fi
...
out="$(SANDBOX_HOME="$T/vh2" "$ROOT/scripts/launch-virtual-sandbox.sh" --port 8000x --dry-run 2>&1)"; rc=$?
if [ $rc -ne 0 ] && echo "$out" | grep -q "拒绝"; then ok "malformed port 8000x rejected fail-closed"; else no "malformed port 8000x slipped guard (rc=$rc): $out"; fi
