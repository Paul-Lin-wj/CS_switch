...
# 铁律：代理端口设成 8000 → 必须失败关闭，输出含 8000
out="$(CSSWITCH_PROXY_PORT=8000 CSSWITCH_CONFIG="$T/nope.json" "$DOCTOR" 2>&1)"; rc=$?
if [ $rc -ne 0 ] && echo "$out" | grep -q "8000"; then ok "doctor fails on reserved port 8000"; else no "doctor did not reject 8000 (rc=$rc): $out"; fi
