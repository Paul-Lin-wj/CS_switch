...
#
# 铁律保障（见 CLAUDE.md）:
#   - 独立 HOME + 独立 data-dir + 独立端口，绝不碰真实 ~/.claude-science 与端口 8000
#   - 通过 `cp -rL` 解引用符号链接后复制运行时资产（bin/conda/runtime/seed-assets），绝不复制任何真实登录凭证
...
APP_BIN="${HOME}/.local/bin/claude-science"
BIN="${SCIENCE_BIN:-}"
PORT=8990
...
if (( 10#${PORT} == 8000 )); then echo "拒绝：端口 8000 是真实实例保留端口"; exit 1; fi
# 用 readlink -m 允许 DATA_DIR 尚不存在；前缀检查保证沙箱 data-dir 不能等于或落在真实目录内。
_dd_real="$(readlink -m "$DATA_DIR")"; _real_real="$(readlink -m "$REAL_DIR")"
_dd_real="${_dd_real%/}"; _real_real="${_real_real%/}"
if [[ "$_dd_real" == "$_real_real" || "$_dd_real" == "$_real_real"/* ]]; then echo "拒绝：data-dir 的真实路径指向真实目录或其子目录"; exit 1; fi
if [[ "$DRY_RUN" == "1" ]]; then echo "DRY-RUN OK：护栏通过，未启动沙箱。"; exit 0; fi

# —— 首次：克隆运行时资产，绝不复制真实登录凭证 ——
if [[ ! -d "$DATA_DIR/bin" ]]; then
  echo "首次初始化沙箱运行时（递归复制并解引用符号链接，只拷运行时、不拷真实登录）…"
  mkdir -p "$DATA_DIR"
...
if [[ "$SKIP_FORGE" == "1" ]]; then
  ...
  node "$PROJ/scripts/make-virtual-oauth.mjs" --auth-dir "$DATA_DIR" --email "$EMAIL" --force
fi
...
echo "  端口     = $PORT   （真实实例 8000 不受影响）"
echo "  二进制   = $BIN"
# 掩掉 proxy-url 里的 path secret（一次性鉴权令牌不入日志）
_masked_proxy="$(printf '%s' "$PROXY_URL" | sed 's#\(://[^/]*/\).*#\1****#')"
