...
classify_port() {  # $1=port；打印占用分型
  local p="$1" cmd
  if [ "$((10#$p))" -eq 8000 ]; then echo "保留端口 8000（真实实例，绝不占用/干预）"; return; fi
  if command -v ss > /dev/null 2>&1; then
    cmd="$(ss -ltnp "sport = :$p" 2>/dev/null | awk 'NR==2{print $6" (pid "$7")"}')"
  fi
  if [ -z "$cmd" ] && command -v lsof > /dev/null 2>&1; then
...
for p in "$PROXY_PORT" "$SANDBOX_PORT"; do
  ...
  if [ "$((10#$p))" -eq 8000 ]; then fail "端口 $p 命中真实实例保留端口 8000（铁律禁用）"; continue; fi
